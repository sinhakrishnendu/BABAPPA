...
BABAPPA `v0.8.0` includes a validated research-alpha deployable bundle built from the retained conservative explicit branch-truth 100K MPS model artifacts. The package is intended for guarded empirical diagnostics with applicability/OOD gates, not final empirical positive-selection discovery.
