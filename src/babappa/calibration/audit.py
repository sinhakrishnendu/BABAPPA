"""Validation utilities for BABAPPA baseline calibration artifacts."""
...
from babappa.calibration.baseline import CALIBRATED_PREDICTION_FIELDNAMES
from babappa.datasets.index import read_tsv
from babappa.models.baseline import VALID_SPLITS


def validate_baseline_calibration_dir(calibration_dir: Union[str, Path]) -> dict:
    """Validate baseline calibration output artifacts."""
    calibration_path = Path(calibration_dir)
    failures: List[str] = []
    warnings: List[str] = []
    calibration_json = calibration_path / "baseline_calibration.json"
    predictions_tsv = calibration_path / "baseline_calibrated_predictions.tsv"
    metrics_json = calibration_path / "baseline_calibrated_metrics.json"

    calibration = _validate_json(
        calibration_json, "baseline_calibration.json", failures
    )
    metrics = _validate_json(
        metrics_json, "baseline_calibrated_metrics.json", failures
    )
    ...
    if isinstance(metrics, dict) and "metrics_by_split_calibrated" not in metrics:
        failures.append("baseline_calibrated_metrics.json missing metrics_by_split_calibrated")
...
def _validate_predictions(path: Path, failures: List[str]) -> List[dict]:
    if not path.exists():
        failures.append(f"missing baseline_calibrated_predictions.tsv: {path}")
        return []
    try:
        rows = read_tsv(path)
    except OSError as exc:
        failures.append(f"unreadable baseline_calibrated_predictions.tsv: {exc}")
        return []

    if rows:
        missing_columns = [
            column for column in CALIBRATED_PREDICTION_FIELDNAMES if column not in rows[0]
        ]
        if missing_columns:
            failures.append(
                "baseline_calibrated_predictions.tsv missing columns: "
